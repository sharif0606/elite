    ...
    @endif
        ...
        <table border="1" width="100%" cellspacing="0">
            ...
            <tbody>
                ...
                @endphp
                @if ($southDetail)
                    @foreach ($southDetail as $de)
                        <tr>
                            <td style="text-align: center;">{{ $sl++  }}</td>
                            <td style="text-align: left;">
                                @if ($de->employee?->en_applicants_name != '')
                                    Name: {{ $de->employee?->en_applicants_name }}
                                @else
                                    Name: {{ $de->employee?->bn_applicants_name }}
                                @endif
                                <br/>
                                @if ($de->employee?->admission_id_no != '')
                                    Office ID: {{ $de->employee?->admission_id_no }} <br/>
                                @endif
                                @if ($de->employee?->bn_nid_no != '')
                                    NID: {{ $de->employee?->bn_nid_no }}
                                @endif
                            </td>
                            <td style="text-align: center;">{{ $de->jobpost?->name }}</td>
                            <td style="text-align: center;">{{ (int) $de->duty_day }}</td>
                            <td style="text-align: center;">{{ money_format($de->net_payment_amount) }}</td>
                            <td style="text-align: center;">{{ money_format($de->net_service_amount) }}</td>
                            <td style="text-align: center;">{{ money_format($de->total_amounts) }}</td>
                            <td style="text-align: center;">{{ $de->remarks }}</td>
                        </tr>
                    @endforeach
                @endif
            </tbody>
                ...
                {{-- @if($invoice_id->vat > 0) --}}
                    <tr style="text-align: center;">
                        ...
                        <td></td>
                        <td>{{ money_format($invoice_id->vat_taka) }}</td>
                        <td></td>
                        <td></td>
                    </tr>
                {{-- @endif --}}
                <tr style="text-align: center;">
                    ...
                    <td></td>
                    <td><b>{{ money_format($invoice_id->grand_total) }}</b></td>
                    <td></td>
                    <td></td>
                </tr>
        </table>
