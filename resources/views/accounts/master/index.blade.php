            ...
            <div class="card">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered mb-0">
                        {{-- <a class="float-end" href="{{route('master.create')}}"style="font-size:1.7rem"><i class="bi bi-plus-square-fill"></i></a> --}}
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            ...
                            <th scope="row">{{ ++$loop->index }}</th>
                                ...
                                {{-- <td class="white-space-nowrap">
                                    <a href="{{route('master.edit',encryptor('encrypt',$d->id))}}">
                                        <i class="bi bi-pencil-square"></i>
                                    </a>
                                    <a href="javascript:void()" onclick="$('#form{{$d->id}}').submit()">
                                        <i class="bi bi-trash"></i>
                                    </a>
                                    <form id="form{{$d->id}}" action="{{route('master.destroy',encryptor('encrypt',$d->id))}}" method="post">
                                        @csrf
                                        @method('delete')
