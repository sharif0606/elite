        ...
        <div class="col-12">
            <div class="card">
                <div class="card-content">
                    <div class="card-body">
                        <form class="form" method="post" action="{{route('sub_head.update',encryptor('encrypt',$sub->id))}}">
                            @csrf
                            @method('patch')
                                ...
                                <div class="col-md-6 col-12">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control form-select" name="master_head" id="master_head">
                                            ...
                                            @forelse($data as $d)
                                                <option value="{{$d->id}}" {{ old('master_head',$sub->master_head_id)==$d->id?"selected":""}}> {{ $d->head_name}}-{{ $d->head_code}}</option>
                                            @empty
