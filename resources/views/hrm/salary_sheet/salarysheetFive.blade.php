...
<script>
                    ...
                    $.each(salary_data, function(index, value) {
                        ...
                        if (!appliedDeductions[value.admission_id_no]) {
                            if (value.charge_status == 0) {
                                ...
                                if (Number(value.cinsurance) > 0 && Number(value.insurance) > 0) {
                                    // If both insurance and cinsurance are greater than 0, take value from value.insurance
                                    Insurance = value.insurance;
                                } else if (Number(value.cinsurance) > 0) {
                                    // If only cinsurance is greater than 0, take value from value.cinsurance
                                    Insurance = value.cinsurance;
                                } else if (Number(value.insurance) > 0) {
                                    // If only insurance is greater than 0, take value from value.insurance
                                    Insurance = value.insurance;
                                } else {
                                    // If neither is greater than 0, set Insurance to 0
                                    Insurance = 0;
                                }
                            ...
                            if (new Date() >= sixMonthsLater && value.charge_status == 0) {
                                ...
                                if (Number(value.cpf) > 0 && Number(value.p_f) > 0) {
                                    // If both cpf and p_f are greater than 0, take value from value.p_f
                                    pf = value.p_f;
                                } else if (Number(value.cpf) > 0) {
                                    // If only cpf is greater than 0, take value from value.cpf
                                    pf = value.cpf;
                                } else if (Number(value.p_f) > 0) {
                                    // If only p_f is greater than 0, take value from value.p_f
                                    pf = value.p_f;
                                } else {
                                    // If neither is greater than 0, set pf to 0
                                    pf = 0;
                                }
    ...
    function total_calculate() {
        var payableTotal = 0;
        var ratOfSalaryTotal = 0;
        var prevDaysTotal = 0;
        var netTotal = 0;
        var otDayTotal = 0;
        var otRateTotal = 0;
        var otAmountTotal = 0;
        var postAlownceTotal = 0;
        var grossTotal = 0;
        var deDressTotal = 0;
        var deFineTotal = 0;
        var deBankChargeTotal = 0;
        var deInsTotal = 0;
        var dePfTotal = 0;
        var deStmpTotal = 0;
        var deTrainingTotal = 0;
        var deLoonTotal = 0;
